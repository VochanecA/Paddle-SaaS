...
// Theme-based conditional logic example
useEffect(() => {
...
// Polling when forceRefresh is true (5s for 30s)
useEffect(() => {
...
// Continuous polling (every 30s, without loading state)
useEffect(() => {
const interval = setInterval(() => {
fetchSubscriptions(false);
}, 30000);
...
const handleCancelSubscription = useCallback(async (subscriptionId: string, immediate = false) => {
...
const CancelModal = useCallback(({ subscriptionId }: { subscriptionId: string }) => (
...
<div  
key={i}  
className="bg-gray-50 dark:bg-gray-800 rounded-lg p-5 border border-gray-200 dark:border-gray-700 animate-pulse"  
>
