...
15. Adapt responses to the user’s expertise level, providing beginner-friendly explanations or advanced insights as appropriate.`,
      ...
      return (
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            {conversation.length > 0 && <span className="bg-white/30 text-xs px-2 py-1 rounded-full">{conversation.length}</span>}
          </div>
        </div>
      );
    }
    ...
    return (
      <div className={`w-full sm:max-w-5xl mx-auto bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 flex flex-col max-h-[80vh] ${className}`}>
        {/* Header */}
          ...
          {conversation.length === 0 ? (
            <div className="text-center text-gray-600 dark:text-gray-300 py-6">
              <div className="w-16 h-16 bg-gradient-to-r from-blue-400/20 to-purple-400/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <Image src="/smiley2.png" alt="Smiley face" width={64} height={64} priority style={{ borderRadius: '9999px' }} />
              </div>
              <p className="text-lg font-medium mb-2">Hello! I&apos;m your AI assistant 😊</p>
              <p className="text-sm mb-6">I can help answer questions, analyze images, and provide insights. How can I assist you today?</p>
            </div>
          ) : (
            conversation.map((msg, index) => (
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[90%] sm:max-w-[80%] p-4 rounded-2xl shadow-md ${msg.role === 'user' ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-br-none' : 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-bl-none'}`}>
                  <ReactMarkdown className="text-sm whitespace-pre-wrap">{msg.content}</ReactMarkdown>
                  <div className={`text-xs mt-2 opacity-70 ${msg.role === 'user' ? 'text-white/70' : 'text-gray-600 dark:text-gray-400'}`}>
            ...
            <div className="flex justify-start">
              <div className="bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white p-4 rounded-2xl rounded-bl-none max-w-[90%] sm:max-w-[80%] shadow-md">
                <div className="flex space-x-1 items-center">
                  ...
                  <div className="w-2 h-2 bg-blue-500/70 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }} />
                  <div className="w-2 h-2 bg-blue-500/70 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }} />
                  <span className="text-xs ml-2 font-medium">{uploadingImage ? 'Analyzing image...' : 'Thinking...'}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-4 sm:p-6 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-x-2">
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={isLoading || !aiAvailable} className="px-3 py-3 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 disabled:bg-gray-400/50 disabled:cursor-not-allowed transition-all shadow-md flex items-center justify-center" title="Upload image">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </button>
            <input ref={inputRef} type="text" value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Ask your AI assistant..." className="flex-1 min-w-0 px-4 py-3 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" disabled={isLoading || !aiAvailable} />
            <button onClick={() => handleSendMessage()} disabled={isLoading || (!message.trim() && !uploadingImage) || !aiAvailable} className="px-4 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg hover:from-blue-600 hover:to-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all transform hover:scale-105 shadow-md" title="Send message">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
              </svg>
            </button>
          </div>
          {!aiAvailable && <p className="text-xs text-red-500 mt-2 text-center">AI service is currently unavailable. Please check your configuration.</p>}
        </div>
      </div>
    );
  }
